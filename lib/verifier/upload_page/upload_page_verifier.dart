...
class UploadPageVerifier extends StatefulWidget {
  const UploadPageVerifier({super.key});

  @override
  State<UploadPageVerifier> createState() => _UploadPageVerifierState();
}

class _UploadPageVerifierState extends State<UploadPageVerifier> {
  final _formKey = GlobalKey<FormState>();
  String priceAmount = '0.0';
  double filePrice = 0.0;

  bool premieumCheckValue = false;
  CollectionReference orgDocuments = FirebaseFirestore.instance
  ...
  Future<void> uploadPdf() async {
    ...
    try {
      DocumentSnapshot documentSnapshot =
          await orgDocuments.doc(pdfFileName).get();
    ...
    await uploadTask.whenComplete(() async {
      ...
      Map<String, dynamic> alertsData = {
        'title': "Organistion Document Uploaded",
        'message': "Organisation file $pdfFileName has been uploaded",
        'status': 'uploaded',
        'uploadedBy': FirebaseAuth.instance.currentUser!.email.toString(),
        'dateTime': Timestamp.fromDate(DateTime.now())
      };
      ...
      CollectionReference userAlerts = FirebaseFirestore.instance
          ...
          .collection(FirebaseAuth.instance.currentUser!.email.toString());
      try {
        await orgDocuments.doc(pdfFileName).set(documentsData);
          ...
          children: [
            ...
            Container(
              ...
              child: Row(
                children: [
                  Checkbox(
                    value: premieumCheckValue,
                    onChanged: ((value) {
                      setState(() {
                        premieumCheckValue = value!;
                      });
                    }),
                    checkColor: kBlack,
                    activeColor: kWhite,
                ...
                child: Column(
                  children: [
                    const Padding(
                      ...
                      child: Text(
                        "Enter the price of the document",
                        textAlign: TextAlign.center,
                        style: TextStyle(fontSize: 16.0),
                      ),
                    ),
                        ...
                        child: TextFormField(
                          ...
                          decoration: const InputDecoration(
                              ...
                              focusedBorder: OutlineInputBorder(
                                  borderSide:
                                      BorderSide(color: kGrey, width: 2.0)),
                              fillColor: kWhite,
                              filled: true,
            ...
            const Padding(
              ...
              child: Text(
                "Upload a Certificate for the users from organisation",
                textAlign: TextAlign.center,
                style: TextStyle(fontSize: 16.0),
              ),
            ),
                ...
                ? InkWell(
                    onTap: () {
                      ...
                      if (_formKey.currentState!.validate()) {
                        double price = double.parse(priceAmount);
                        setState(() {
                          filePrice = price;
                        });
                      }
                    },
                    child: SelectPDFButton('Upload a Certificate'))
                : InkWell(
                    onTap: uploadPdf, child: UploadPDFButton(pdfFileName)),
            Padding(
